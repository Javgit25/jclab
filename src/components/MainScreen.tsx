                    ...
                    {bios.map((b: any, i: number) => {
                      ...
                      return (
                        ...
                        <tr style={{ background: rowBg, borderLeft: isUrgent ? '3px solid #dc2626' : 'none', borderBottom: hasExtra ? 'none' : '1px solid #e2e8f0' }}>
                          ...
                          <td style={{ padding: '5px 4px', textAlign: 'center' }}>{(b.type === 'IHQ' || b.tissueType === 'Inmunohistoquímica') ? (() => { const tpc = b.trozoPorCassette || []; const cns = b.cassettesNumbers || []; const tv = tpc.length > 0 ? tpc.reduce((s: number, v: number) => s + (v || 1), 0) : (parseInt(b.cassettes) || 0); return <div><span style={{ fontWeight: 700 }}>{tv} vid.</span>{tpc.length > 1 && <div style={{ fontSize: '6pt', color: '#93c5fd' }}>{tpc.map((t: number, ci: number) => { const c = cns[ci]; return (c?.suffix ? c.base + '/' + c.suffix : 'C' + (ci+1)) + ':' + (t || 1); }).join(' · ')}</div>}</div>; })() : isPAP || isCito || b.type === 'TC' || b.tissueType === 'Taco en Consulta' ? '-' : (() => { const tpc = b.trozoPorCassette || []; const cns = b.cassettesNumbers || []; const totalT = tpc.length > 0 ? tpc.reduce((s: number, v: number) => s + (v || 1), 0) : (parseInt(b.pieces) || 0); if (tpc.length > 1) { return <div><span style={{ fontWeight: 700 }}>{totalT}</span><div style={{ fontSize: '6pt', color: '#94a3b8' }}>{tpc.map((t: number, ci: number) => { const c = cns[ci]; return (c?.suffix ? c.base + '/' + c.suffix : 'C' + (ci+1)) + ':' + (t || 1); }).join(' · ')}</div></div>; } return totalT || '-'; })()}</td>
                        </tr>
